...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if ('serviceWorker' in navigator && 'PushManager' in window) {
      try {
        ...
        const subscription = await registration.pushManager.subscribe({
          ...
          applicationServerKey: urlBase64ToUint8Array('TU_CLAVE_PUBLICA_VAPID') // Reemplaza con tu clave pública VAPID
        });
        ...
        await fetch('/api/subscribe-push', {
          ...
          body: JSON.stringify({
            ...
            user_id: 'ID_DEL_USUARIO', // Obtener dinámicamente
            role: 'admin'
          })
        });
        console.log('Suscripción a push enviada');
      } catch (error) {
  ...
  // Manejar actualización de estado del pedido para el cliente
  socket.on('order_status_update', function(order) {
    console.log('Evento recibido en cliente:', order);
    ...
    if (pedido) {
      ...
      if (Notification.permission === 'granted') {
        ...
        if (order.status === 'en-camino') {
          message = `El pedido #${order.order_id} está en camino. Motorizado: ${order.motorizado_name}`;
        } else if (order.status === 'entregado') {
          message = `El pedido #${order.order_id} ha sido entregado`;
        } else {
          message = `El estado del pedido #${order.order_id} ha cambiado a ${newStatus}`;
        }
