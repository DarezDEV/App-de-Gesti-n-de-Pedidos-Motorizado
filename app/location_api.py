...
def save_location_to_file(order_id, data):
    ...
    if os.path.exists(filepath):
        try:
            ...
            logger.error(f"Error leyendo archivo de ubicaciones: {e}")
    ...
    try:
        ...
        logger.error(f"Error guardando archivo de ubicaciones: {e}")

# Función para actualizar la ubicación en la base de datos
def update_location_in_db(order_id, role, lat, lon):
    try:
        ...
        logger.error(f"Error al actualizar ubicación en la BD: {e}", exc_info=True)
        return False

# Función para emitir actualización de ubicación por Socket.IO
def emit_location_update(order_id, location_data):
    try:
        if hasattr(current_app, 'socketio'):
            ...
            logger.debug(f"Ubicación emitida por Socket.IO: {order_id}, {location_data['role']}")
        return True
    except Exception as e:
        logger.error(f"Error al emitir evento Socket.IO: {e}", exc_info=True)
        return False

# Endpoint para suscribirse a notificaciones push
@location_api.route('/api/subscribe-push', methods=['POST'])
@handle_errors
def subscribe_push():
    data = request.json
...
@location_api.route('/api/update-location', methods=['POST'])
@handle_errors
def update_location():
    data = request.json
    ...
    # Guardar en archivo como respaldo (asincrónicamente si es posible)
    save_location_to_file(order_id, data)
    
    # Actualizar la base de datos
    update_location_in_db(order_id, role, lat, lon)
    
    # Emitir evento a través de Socket.IO
    emit_location_update(order_id, location_data)
    
    return jsonify({'success': True}), 200

# Endpoint para obtener las últimas ubicaciones conocidas
@location_api.route('/api/get-locations/<order_id>', methods=['GET'])
@handle_errors
def get_locations(order_id):
...
def handle_get_initial_locations(data=None):
    ...
    if not locations:
        try:
            ...
            logger.error(f"Error al cargar ubicaciones desde la BD: {e}", exc_info=True)
...
def register_socketio_events(socketio):
    ...
    def on_get_initial_locations(data):
        handle_get_initial_locations(data)
    ...
    def on_update_location(data):
        ...
        try:
            ...
            # Validación básica de coordenadas
            if not (-90 <= lat <= 90) or not (-180 <= lon <= 180):
                logger.warning(f"Coordenadas inválidas recibidas: {lat}, {lon}")
                return
            ...
            logger.error(f"Error en Socket.IO update_location: {e}", exc_info=True)
...
def start_background_tasks():
    ...
    def cleanup_task():
        while True:
            try:
                logger.debug("Ejecutando limpieza de ubicaciones antiguas")
...
@location_api.route('/admin/dashboard-data', methods=['GET'])
@handle_errors
def get_dashboard_data():
    # Implementación con datos reales desde la base de datos
    try:
        with get_db_connection() as conn:
            with conn.cursor(dictionary=True) as cursor:
...
def init_app(app):
    ...
    logger.info("Módulo location_api inicializado")